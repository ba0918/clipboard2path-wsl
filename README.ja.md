...
1. クリップボードを監視 — X11 XFixes の所有者変更通知によるイベント駆動。X11 が使えない環境ではポーリング（`wl-paste` のみ使用）にフォールバック
2. 画像（BMP）を検知したら取得
...
WATCH OPTIONS:
    --once              単発実行（デーモンループなし）
    --poll              ポーリングモードを強制（デフォルト: X11 イベント駆動、ポーリングフォールバック）
...
```bash
cargo build --release    # リリースビルド（~800KB）
cargo test               # テスト実行（233 tests）
cargo clippy             # リント
```
...
src/
  ...
  infra/                   # I/O 層（トレイトで抽象化）
    change_signal.rs       #   イベント駆動の変更検知（X11 XFixes）
