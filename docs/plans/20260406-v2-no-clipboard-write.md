...
- [x] `domain/runtime_dir.rs`: ランタイムディレクトリのパス解決（純粋関数）
...
- [x] `domain/mod.rs` に `pub mod runtime_dir;` を追加
- [x] `infra/lifecycle.rs`: デーモンライフサイクル（トレイト + 実装）
  ```rust
...
- [x] `infra/mod.rs` に `pub mod lifecycle;` を追加
- [x] `main.rs`: デーモン起動前に `DaemonLifecycle::setup()` を呼び出し
- [x] SIGTERM/SIGINT ハンドリング: `ctrlc` クレートで teardown を実行
...
- [x] `infra/clipboard.rs` から `ClipboardWriter` トレイトと `WlClipboard` の `ClipboardWriter` 実装を削除
  - `MockWriter` 関連テストも削除
- [x] `service/converter.rs` の `ConvertService` からジェネリクスパラメータ `W: ClipboardWriter` を除去
...
- [x] `service/daemon.rs` の `poll_once` ジェネリクスから `W: ClipboardWriter` を除去し、`N: PathNotifier` に置換
- [x] `main.rs` の DI 組み立て（`ConvertService::new(...)` 呼び出し）を更新
  - `WlClipboard`（Writer用）を `FilePathNotifier` に置換
- [x] `infra/path_notifier.rs`: パス通知（トレイト + 実装）
  ```rust
...
- [x] `infra/mod.rs` に `pub mod path_notifier;` を追加
- [x] `service/converter.rs` の `AppError` に `Notify(NotifyError)` バリアントを追加 + `From<NotifyError>` 実装

### Step 1.3: 自己トリガー防止の簡素化
- [x] `wl-copy` を使わなくなったので、デバウンス機構は **不要になる可能性が高い**
...
### Step 1.4: 保存先の変更
- [x] デフォルト保存先: `/tmp/` → `$XDG_RUNTIME_DIR/clipboard2path/`
...
- [x] `--output-dir` オプションは引き続きサポート（明示指定で上書き可能）
- [x] `help_text()` のデフォルト値表示を更新
- [x] ファイル名形式は変更なし: `clipboard-{timestamp}.png`

### Step 1.5: ローテーション見直し
- [x] `domain/cleanup.rs`: デフォルト保持上限を変更
  - 最大保持数: 100 → **20**
  - 年齢ベースクリーンアップ: 24時間 → **不要**（件数ベースのみで十分）
  - `files_to_clean_by_age()` 関数を削除（使用箇所なくなるため）
  - 最古のファイルから削除
- [x] `main.rs` の `run_cleanup()` から年齢ベース削除ロジックを除去（件数ベースのみ残す）
- [x] `--max-files` デフォルト値を `domain/cli.rs` で 20 に更新

### Step 1.6: テスト更新
- [x] `infra/clipboard.rs` の `MockWriter` / `ClipboardWriter` 関連テストを削除
- [x] `infra/path_notifier.rs` のテストを追加（`MockPathNotifier` + `FilePathNotifier` 統合テスト）
- [x] `service/converter.rs` のテストを `PathNotifier` ベースに書き換え
  - `MockClipboardWriter` → `MockPathNotifier` に置換
  - `convert_once_writes_path_to_clipboard` → `convert_once_notifies_path` に書き換え
- [x] `service/daemon.rs` のテストから `ClipboardWriter` 依存を除去、`PathNotifier` に置換
- [x] `domain/debounce.rs` のテストを削除（ファイルごと削除するため）
- [x] `domain/cleanup.rs` の `files_to_clean_by_age` 関連テストを削除
- [x] `cargo test` + `cargo clippy` 全パス
...
### Step 2.1: シェル検出
- [x] `domain/shell_detect.rs`: 現在のシェルを判定（純粋関数）
  - `fn detect_shell(shell_env: &str) -> ShellType`
  - `$SHELL` 環境変数から fish / bash / zsh を判定
  - テスト: 各シェルパス → 正しい ShellType
- [x] `domain/mod.rs` に `pub mod shell_detect;` を追加

### Step 2.2: シェルフック生成
- [x] `domain/shell_hook.rs`: シェルごとのフックスクリプト生成（純粋関数）
...
- [x] `domain/mod.rs` に `pub mod shell_hook;` を追加

### Step 2.3: CLI サブコマンド対応
- [x] `domain/cli.rs` をサブコマンド対応に拡張
...
- [x] `clipboard2path-wsl init [fish|bash|zsh]`
  - シェル指定なし → 自動検出
  - フックスクリプトを適切な場所に配置:
  ...
  - `--force` で強制上書き
- [x] `clipboard2path-wsl uninstall` — フックを削除

### Step 2.5: テスト
- [x] シェル検出テスト
- [x] フック生成テスト（各シェル）
- [x] CLIサブコマンドパーステスト（後方互換含む）
- [x] initコマンドの統合テスト（モック注入でファイル書き込み検証）
...
### Step 3.1: バイナリサイズ最適化
- [x] release profile は既存のまま（`opt-level = "z"`, `lto = true`, `strip = true`）
- [x] `wl-copy` 関連コード削除によるサイズ削減を確認
- [x] 実測: 668KB（ctrlc クレート追加分で微増、v1: 593KB）

### Step 3.2: systemd サービス更新
- [x] `clipboard2path.service` を更新
  - `ExecStop` で teardown（SIGTERM でクリーンアップ）を確認
  - `Environment=XDG_RUNTIME_DIR=/run/user/%U` を確認

### Step 3.3: README 更新
- [x] 新しいセットアップ手順（`clipboard2path-wsl init` の説明）
- [x] クリップボード非書き換えの設計思想を記載
- [x] Windows 側への影響がないことを明記

### Step 3.4: Wiki 更新
- [x] アーキテクチャ概要の更新（ClipboardWriter削除、PathNotifier追加）
- [x] WSL2 クリップボード連携記事の更新（双方向同期の問題と対策）
...
- [x] Phase 1: クリップボード非書き換えで画像→PNG→パスファイル更新が動作する
- [x] Phase 2: `clipboard2path-wsl init` 後、Ctrl+V でパスが入力される。Slack等のWindows貼り付けが壊れない
- [x] Phase 3: README + Wiki が更新され、新設計が文書化されている
